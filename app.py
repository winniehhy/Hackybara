...
class AuditLogger:
    ...
    def log_operation(self, operation_type, file_id=None, details=None, status='success', duration=None):
        """
        Log an operation to the audit trail
        
        Args:
            operation_type: Type of operation (e.g., 'file_upload', 'text_extraction', 'pii_detection')
            file_id: Associated file ID if applicable
        ...
        try:
            with self.db_manager.get_db_connection() as conn:
                ...
                cursor.execute('''
                    ...
                    (operation_type, file_id, operation_details, status, duration, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (
                    operation_type,
                    file_id,
                ...
                conn.commit()
                print(f"Audit log: {operation_type} - {status}")
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with self.get_db_connection() as conn:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_audit_logs_operation_type ON audit_logs (operation_type)')
    ...
    def get_audit_logs(self, limit=100, offset=0, file_id=None, operation_type=None):
        """Retrieve audit logs with optional filtering"""
        with self.get_db_connection() as conn:
            cursor = conn.cursor()
            
            query = '''
                SELECT id, operation_type, file_id, operation_details, status, 
                       duration, timestamp
                FROM audit_logs
            '''
            params = []
    ...
    def get_audit_trail_for_file(self, file_id):
        """Get complete audit trail for a specific file"""
        with self.get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT operation_type, operation_details, status, duration, timestamp
                FROM audit_logs
                WHERE file_id = ?
...
def process_pii_detection(file_id, extracted_text):
    ...
    if not PII_DETECTION_AVAILABLE:
        audit_logger.log_operation(
            'pii_detection', 
            file_id=file_id, 
            details={'error': 'PII Detection module not available'}, 
            status='error'
        )
    ...
    try:
        ...
        audit_logger.log_operation(
            ...
            file_id=file_id, 
            details={'text_length': len(extracted_text), 'model': 'gemma3'}
        )
        ...
        audit_logger.log_operation(
            ...
            file_id=file_id, 
            details={
                'pii_instances_found': len(matches),
                'total_pii_found': summary.get('total_pii_found', 0),
        ...
        audit_logger.log_operation(
            'file_saved', 
            file_id=file_id, 
            details={'output_path': output_path, 'file_type': 'pii_results_json'}
        )
...
def extract_text():
    try:
        if 'file' not in request.files:
            audit_logger.log_operation('file_upload', details={'error': 'No file provided'}, status='error')
            return jsonify({'error': 'No file provided'}), 400

        file = request.files['file']
        if file.filename == '':
            audit_logger.log_operation('file_upload', details={'error': 'No file selected'}, status='error')
...
def get_document(file_id):
    ...
    try:
        audit_logger.log_operation(
            'document_retrieval', 
            file_id=file_id, 
            details={'operation': 'get_document_info'}
        )
...
def get_pii_results(file_id):
    ...
    try:
        audit_logger.log_operation(
            'pii_results_retrieval', 
            file_id=file_id, 
            details={'operation': 'get_pii_results'}
        )
        ...
        if not pii_results:
            audit_logger.log_operation(
                'pii_results_retrieval', 
                file_id=file_id, 
                details={'error': 'PII results not found'}, 
                status='error'
            )
...
def save_pii_results():
    try:
        ...
        audit_logger.log_operation(
            'pii_review_save', 
            file_id=file_id, 
            details={'data_size': len(reviewed_data)}
        )
        ...
        audit_logger.log_operation(
            'pii_review_saved', 
            file_id=file_id, 
            details={'operation': 'save_to_reviewed_table'}
        )
...
def get_audit_logs():
    ...
    try:
        ...
        for log in logs:
            log_dict = dict(log)
            # Parse operation_details if it's JSON
...
@app.route('/audit/trail/<file_id>', methods=['GET'])
def get_file_audit_trail(file_id):
    """Get complete audit trail for a specific file"""
    try:
        audit_logger.log_operation(
            'audit_trail_retrieval', 
            file_id=file_id, 
            details={'operation': 'get_complete_trail'}
        )

        trail = db_manager.get_audit_trail_for_file(file_id)
        
        # Convert to list of dictionaries and parse details
        trail_data = []
        for entry in trail:
            entry_dict = dict(entry)
            if entry_dict['operation_details']:
                try:
                    entry_dict['operation_details'] = json.loads(entry_dict['operation_details'])
                except json.JSONDecodeError:
                    pass
            trail_data.append(entry_dict)

        return jsonify({
            'file_id': file_id,
            'audit_trail': trail_data,
            'total_operations': len(trail_data)
        })
...
def export_audit_logs():
    ...
    try:
        # Get all audit logs
        logs = db_manager.get_audit_logs(limit=10000)  # Large limit for export
        ...
        for log in logs:
            ...
            if log_dict['operation_details']:
                try:
                    ...
                    pass
            export_data['audit_logs'].append(log_dict)
        
        # Save to file
        export_filename = f"audit_logs_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
